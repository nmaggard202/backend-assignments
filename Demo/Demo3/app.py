...
def create_subtask(task_id):
    ...
    task = DB.get_task_by_id(task_id)
    if task is None:
        return failure_response("Parent task wasn't found.")
...
def get_subtasks_of_task(task_id):
    """Endpoint for getting all subtasks for a task with 'task_id'"""
    return success_response({"subtasks": DB.get_subtasks_of_task(task_id)})


@app.route("/subtasks/")
def get_subtasks():
    """Endpoint for getting all the subtasks."""
